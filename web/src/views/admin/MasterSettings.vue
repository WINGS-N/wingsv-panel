<template>
  <div class="client-page">
    <SamsungCard kicker="Master" title="Общие настройки клиентов">
      <p class="body-copy">
        Здесь хранится конфигурация, которую можно массово раскатать на всех ваших клиентов одной кнопкой. Уникальные
        поля (Xray-профили и подписки, ключи WireGuard, комната WB Stream, аватар, sharing-настройки) никогда не
        перезаписываются — обновятся только секции, отмеченные ниже.
      </p>
    ...
    <SamsungCard v-if="!loading && !loadError" title="Заполнить из источника">
      <p class="body-copy">
        ...
        ссылки — уникальные поля (Xray-профили и подписки, ключи WG, комната WB Stream, sharing/xposed) автоматически
        выбрасываются.
      </p>
      <div class="mt-3">
    ...
    <SamsungCard v-if="!loading && !loadError" title="Шаблон конфигурации">
      <p class="body-copy">
        Заполните только те секции, которые включены выше. Остальные значения тут можно оставить пустыми — панель не
        тронет их при применении.
      </p>
      <ConfigFormEditor :model-value="formValue" :sections="visibleSections" @update:model-value="onFormChanged" />
    </SamsungCard>
...
const enabledScopeLabels = computed(() => scopeOptions.filter((o) => scopeFlags[o.id]).map((o) => o.label));
