...
type Guardian struct {
	...
	// SPKI pins (raw 32-byte SHA-256 of the panel CA SubjectPublicKeyInfo) the app
	// must accept for the panel's control-plane TLS (Guardian WSS + panel gRPC),
	// used when the panel has no public certificate. Empty means verify against
	// the system trust store (a public / ACME certificate). Multiple entries allow
	// CA rotation with overlap. This never applies to the VK TURN DTLS relay.
	ServerCaPins  [][]byte `protobuf:"bytes,10,rep,name=server_ca_pins,json=serverCaPins,proto3" json:"server_ca_pins,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Guardian) GetServerCaPins() [][]byte {
	if x != nil {
		return x.ServerCaPins
	}
	return nil
}
...
const file_wingsv_proto_rawDesc = "" +
	...
	"\x0econfig_version\x18\x11 \x01(\x04R\rconfigVersion\"\x8f\x03\n" +
	"\bGuardian\x12\x15\n" +
	...
	"\x14admin_avatar_version\x18\t \x01(\x03R\x12adminAvatarVersion\x12$\n" +
	"\x0eserver_ca_pins\x18\n" +
	" \x03(\fR\fserverCaPins\"\x9f\x02\n" +
	"\bWbStream\x12\x17\n" +
