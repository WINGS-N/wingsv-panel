...
// Код приглашения короткий, и его подбор - это просто цикл по HEX. Ограничение
// попыток и есть то, чем этот перебор ломается: восемь байт при десятке попыток
// в минуту не перебрать никогда.
const (
	redeemAttempts = 10
...
//
// Держится в памяти: панель живёт одним процессом, а переживать её рестарт
// счётчику незачем - перебор всё равно упирается в новое окно.
...
//
// Ставится на всё, где подбирают секрет: пароль, код второго фактора, одноразовый
// код приложения, код приглашения. Без него подбор упирается только в скорость
// сети, а это не преграда вообще
