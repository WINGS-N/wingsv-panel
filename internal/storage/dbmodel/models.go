...
type InviteToken struct {
	...
	//
	// Тега default тут нет намеренно: gorm считает Go-ноль "не задано" и молча
	// подставляет дефолт колонки, из-за чего безлимитный код превращался в
	// одноразовый. Значение задаётся явно во всех местах вставки
	MaxUses int64 `gorm:"column:max_uses;not null"`
